...
import { getSetById, saveStudyHistory, saveSessionState, getSessionState } from '@/utils/indexedDB';
...
const ClassificationQuiz = ({ onFinish, onBack, setId, title, quizType, sessionState }) => {
  ...
  useEffect(() => {
    if (shuffledItems.length > 0 && currentItemIndex < shuffledItems.length) {
